        ...
        public void Run()
        {
            ...
            for (int y = 0; y < 16; y++)
            {
                ...
                for (int x = 0; x < 16; x++)
                {
                    ...
                    {
                        Console.WriteLine(i);
                        currentnumber *= grid[y][x + i]; //horizontal   
                    }
                    if (currentnumber > biggestNumber)
                    {
                        biggestNumber = currentnumber;
                        Console.WriteLine("Found biggernumber: " + biggestNumber);
                    }

                    currentnumber = 1;
                    for (int i = 0; i < 4; i++)
                    {
                        currentnumber *= grid[y + i][x]; //horizontal   
                    }
                    if (currentnumber > biggestNumber)
                    {
                        biggestNumber = currentnumber;
                    ...
                    currentnumber = 1;
                    if (x >= 3 && y <= 16) //diagonal left
                    {
                        for (int i = 0; i < 4; i++)
                        {
                            currentnumber *= grid[y + i][x - i];
                        }
                        if (currentnumber > biggestNumber)
                        {
                            biggestNumber = currentnumber;
